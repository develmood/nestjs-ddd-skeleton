...
import {OnboardingEvents, Password, User, VerificationCode} from "@nestjs-ddd-skeleton/onboarding/domain";
...
export class SignUpService {
  ...
  async verifyEmail(verificationCode: number): Promise<boolean> {
    const user = await this.dataService.findByVerificationCode(verificationCode);
